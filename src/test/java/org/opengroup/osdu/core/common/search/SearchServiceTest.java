...
public class SearchServiceTest {
    ...
    public void constructorTest() {
        SearchService searchService = new SearchService(config, httpClient, dpsHeaders);
    ...
    @Test
    public void should_return_valid_response_when_searchCursor_response_is_valid() throws SearchException {
        HttpResponse httpResponse = mock(HttpResponse.class);
        SearchService searchService = new SearchService(config, httpClient, dpsHeaders);
        when(httpResponse.isSuccessCode()).thenReturn(true);
        when(httpResponse.parseBody(CursorQueryResponse.class)).thenReturn(new CursorQueryResponse());
        when(httpClient.send(any(HttpRequest.class))).thenReturn(httpResponse);
        CursorQueryResponse cursorQueryResponse = searchService.searchCursor(new CursorQueryRequest());
    ...
    @Test
    public void should_return_valid_response_when_search_response_is_valid() throws SearchException {
        HttpResponse httpResponse = mock(HttpResponse.class);
        SearchService searchService = new SearchService(config, httpClient, dpsHeaders);
        when(httpResponse.isSuccessCode()).thenReturn(true);
        when(httpResponse.parseBody(QueryResponse.class)).thenReturn(new QueryResponse());
        when(httpClient.send(any(HttpRequest.class))).thenReturn(httpResponse);
        QueryResponse queryResponse = searchService.search(new QueryRequest());
